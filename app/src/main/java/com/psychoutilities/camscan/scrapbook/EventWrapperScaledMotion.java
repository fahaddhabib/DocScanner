...
public class EventWrapperScaledMotion {
    ...
    private static StateTransform startTransformState;
    ...
    public EventWrapperScaledMotion(MotionEvent motionEvent, float mScale, float mOffsetX, float mOffsetY) {
    ...
    private void saveTransformState() {
        startTransformState = new StateTransform(this);
    ...
    public DiffTransform getTransformDifference() {
    ...
    private static class StateTransform {
        private final boolean hasFixedCenterPoint;
        private final List<Point> points = new ArrayList();

        public StateTransform(EventWrapperScaledMotion scaledMotionEventWrapper) {
        ...
        public DiffTransform calculateDiff(EventWrapperScaledMotion scaledMotionEventWrapper) {
            StateTransform transformState = new StateTransform(scaledMotionEventWrapper);
            Point centerPoint = getCenterPoint();
            Point centerPoint2 = transformState.getCenterPoint();
            return new DiffTransform(transformState.getDistance() - getDistance(), transformState.getAngle() - getAngle(), centerPoint2.x - centerPoint.x, centerPoint2.y - centerPoint.y, transformState.getDistance() / getDistance());
        }
    }

    public static class DiffTransform {
        ...
        public DiffTransform(float distanceDiff, float angleDiff, float xDiff, float yDiff, float scale) {
    ...
    private static float pxToDp(float f) {
        return f / DisplayLocal.SCREEN_DENSITY;
    }
