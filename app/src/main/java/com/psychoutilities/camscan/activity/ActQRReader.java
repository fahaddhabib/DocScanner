...
import com.psychoutilities.camscan.qrcode_reader.ScannerViewZXing;
...
public class ActQRReader extends BaseAct implements ScannerViewZXing.HandlerResult {
    ...
    public ScannerViewZXing zXingScannerView;

    @Override
    public void onCreate(Bundle bundle) {
        super.onCreate(bundle);
        setContentView(R.layout.qrreader_act);
        init();
    }

    private void init() {
        viewGroup = (ViewGroup) findViewById(R.id.fl_camera);
        zXingScannerView = new ScannerViewZXing(this);
    ...
    public void setupBarcodeFormats() {
        ...
        if (selected_indices == null || selected_indices.isEmpty()) {
            ...
            for (int i = 0; i < ScannerViewZXing.ALL_FORMATS.size(); i++) {
                selected_indices.add(Integer.valueOf(i));
            }
        }
        Iterator<Integer> it = selected_indices.iterator();
        while (it.hasNext()) {
            arrayList.add(ScannerViewZXing.ALL_FORMATS.get(it.next().intValue()));
        }
    ...
    public void handleResult(Result result) {
        ...
        dialog.setContentView(R.layout.barcode_result_dialog_act);
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(0));
        dialog.getWindow().setLayout(-1, -2);
        dialog.setCanceledOnTouchOutside(false);
        dialog.setCancelable(false);
        if (AdsAdmob.SHOW_ADS) {
            AdsAdmob.loadNativeAds(ActQRReader.this, (View) null, (ViewGroup) dialog.findViewById(R.id.admob_native_container), (NativeAdView) dialog.findViewById(R.id.native_ad_view));
        } else {
        ...
        ((ImageView) dialog.findViewById(R.id.iv_close)).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Toast.makeText(ActQRReader.this, getResources().getString(R.string.text_copied_to_clipboard), Toast.LENGTH_SHORT).show();
                if (zXingScannerView == null) {

                    zXingScannerView = new ScannerViewZXing(ActQRReader.this);
                    viewGroup.addView(zXingScannerView);
                }
                zXingScannerView.setResultHandler(ActQRReader.this);
