...
import com.psychoutilities.camscan.activity.ActGroupDoc;
...
public class AdapterGroupDoc extends RecyclerView.Adapter<AdapterGroupDoc.HolderView> {

    public Activity activity;
    private ArrayList<ModelDB> arrayList;

    public AdapterGroupDoc(Activity activity2, ArrayList<ModelDB> arrayList2) {
        this.activity = activity2;
        this.arrayList = arrayList2;
    }

    @Override
    public void onBindViewHolder(final HolderView viewHolder, final int i) {
        if (i < (arrayList.size() - 1)) {
            ...
            viewHolder.iv_doc.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ((ActGroupDoc) activity).onClickSingleDoc(i);
                }
            });
            viewHolder.iv_doc_item_more.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    ((ActGroupDoc) activity).onClickItemMore(i, viewHolder.tv_doc_name.getText().toString());
                }
            });
    ...
    public HolderView onCreateViewHolder(ViewGroup viewGroup, int i) {
        return new HolderView(LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_list_group_doc_act, viewGroup, false));
    }
